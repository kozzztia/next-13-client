...
// const getAllTodos = async (param: string) : Promise<todoType[]> => {
//     const data = await fetch(`https://jsonplaceholder.typicode.com/todos?search=${param}`);
//     return data.json()
// }

type Props = {
    onSearch : (value : todoType[])=> void;
...
const TodoSearch = ({onSearch , onLoading} : Props) => {
    const {inputPlaceholder} = text
    const [searchValue , setSearchValueValue] = useState<string>("");
    const handleSubmit = async (e  : FormEvent<HTMLFormElement>, )=>{
        await e.preventDefault()
        await onLoading(true)
        await getSearchedTodos(searchValue).then(data => onSearch(data)).finally(()=>onLoading(false))
